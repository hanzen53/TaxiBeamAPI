...
// var map = new L.Map('map', {center: new L.LatLng( 13.753,100.571 ), zoom: 9});
// // var googleLayer = new L.Google('ROADMAP');
// // map.addLayer(googleLayer);
// var googleMap = new L.TileLayer('https://mt.google.com/vt/lyrs=m&hl=en&x={x}&y={y}&z={z}')
// map.addLayer(googleMap);

var map = L.map('map').setView([13.753,100.571], 13);
...
device_id = device_id; // device_id from Ecadmin;
		...
		if (FocusTaxiID2){
			...
			$.ajax({
				...
				success: function(data){				
					...
					if (data.data.allowpsgcontact=='Y') {
						$('.taxibeampsng-statusgroup-tel').find('i.icon-icn_phone').css('visibility', 'visible');
						$('.taxibeampsng-taxidetail-taxitel').html('<a href="tel:'+data.data.phone+'">'+data.data.phone+'</a>');				
					} else {
					...
					if (data.data.carreturn=="Y"){
						...
						$('.taxibeampsng-status-content.taxicolor p').html('ส่งรถ<br>'+data.data.carreturnwhere);
					} else {
						$('.taxibeampsng-taxicolor').attr('src','../assets/img/'+data.data.cartype+'_'+getCarcolorCode(data.data.carcolor)+'@2x.png');
...
function getDrvarround( curlng, curlat){
	...
	if ( getDrvarroundLat > 0) {
		$.ajax({
			...
			dataType: "json", 
			data: " { \"favcartype\": "+JSON.stringify(getfavcartype())+", \"curlng\": "+getDrvarroundLng+", \"curlat\": "+getDrvarroundLat+", \"radian\":10000, \"amount\":30 } ", 
			...
			*/ 
			...
			if(item.curlat!=''&&item.curlng!=''){
				...
				TaxiCurLayer[tempmk].on('click', function(e) {
					...
					// console.log(FocusTaxiID)
					// console.log(FocusTaxiID2)
					...
					if($('.taxibeampsng-info-sec .wrap').attr('data-ref') !== FocusTaxiID2) {
						...
						if(this.options.itemData.carreturn=='Y') {
							$('.taxibeampsng-taxiplatestatus').html(''+this.options.itemData.carplate+' [ ส่งรถ ]');
						} else {
						...
						if (this.options.itemData.allowpsgcontact=='Y') {
							$('.taxibeampsng-statusgroup-tel').find('i.icon-icn_phone').css('visibility', 'visible');
							$('.taxibeampsng-status-taxitel').html('<a href="tel:'+this.options.itemData.phone+'"><i class="icon-icn_phone"></i> '+this.options.itemData.phone+'</a>');
						} else {
							$('.taxibeampsng-statusgroup-tel').find('i.icon-icn_phone').css('visibility', 'hidden');
						...
						if (FocusTaxiID!=FocusTaxiID2) {
							...
							if(display == 'block') {	
								...
								//FocusTaxiID = "";
								FocusTaxiID2 = "";
								//console.log('ggg - '+FocusTaxiID)
								//console.log('hhh - '+FocusTaxiID2)								
								getDrvarround( curlng, curlat);
							} else {
								$('.taxibeampsng-info-sec .wrap').show();
					 			var icon = L.icon({
								...
								this.setIcon(icon);
								//console.log('iii - '+FocusTaxiID)
								//console.log('jjj - '+FocusTaxiID2)
								getDrvarround( curlng, curlat);
							}
						}
					}
...
function checkCenterFirst(callback){	
	if(navigator.geolocation) {		
		navigator.geolocation.getCurrentPosition(function(position) {
			console.log('check center first')
	      		curlng =  position.coords.longitude ;
	      		curlat =  position.coords.latitude ;			
	      		JsonData = { "type": "Point", "coordinates": [position.coords.longitude, position.coords.latitude], "lat": position.coords.latitude, "lng": position.coords.longitude, "time stamp" : [new Date(position.timestamp)], "tempid" : tempid }
...
function checkPsgRequest() {
	...
	if ( $('#psgfrom').val()=='' ) {
		alert('กรุณากรอกข้อมูลต้นทาง<br>กำหนดจุดรับจากแผนที่')
		return false;
	} else if ($('#psgto').val()=='') {
	...
	if ( $('#psgfrom').val()==''  || $('#psgto').val()=='' || $('#psgphone').val().length <= 8 ){				
		...
		if ( favcartype==''  ){
			...
			if ($('#psgfrom').val() =='ใช้ตำแหน่งปัจจุบัน') {
				curaddr_add = "จุดผู้โดยสารบนแผนที่";
			} else {
						...
						case "WAIT":
							...
							$.ajax({
								...
								success: function(result){
									...
									$('.taxibeampsng-gottaxi-taxino').text( result.data.carplate);
									$('.taxibeampsng-gottaxi-taxiname').text(result.data.fname + ' ' + result.data.lname);
									if(result.data.allowpsgcontact=="Y"){
										$('.taxibeampsng-statusgroup-tel').find('i.icon-icn_phone').css('visibility', 'visible');
										$('.taxibeampsng-gottaxi-taxitel span').html('<i class="icon-icn_phone"></i><a href="tel:'+result.data.phone+'">'+result.data.phone+'</a>');
									} else {  
										$('.taxibeampsng-statusgroup-tel').find('i.icon-icn_phone').css('visibility', 'hidden');
...
function addPoiJson(JsonData) {
	...
	if (distance(PsgOldLat, PsgOldLng, PsgNewLat, PsgNewLng, "K") > .001) {
		...
		//map.setView([ JsonData.coordinates[1],JsonData.coordinates[0] ], 20);
		map.addLayer(putPsg);
	}	
...
function cancelCall() {		
		$.ajax({
			...
			success: function(data){
				...
				if (PsgstartLat) {
					map.setCenter( [PsgstartLat, PsgstartLng ]) ;	
				}
				checkstatuspsg();
			        		if(map.hasLayer(putPsg)) {
			        			map.removeLayer(putPsg);		        			
